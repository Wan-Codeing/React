...
/*
    ...
    이런점 때문에 명확한 자료형으로 개발을 해야하는 타입스크리트가 각광받고있는 추세인데
    반드시 자바스크립트로 개발을 해야만 하는 상황에서는 propTypes를 활용해서 해결할수 있다

    PropTypes는 부모로부터 전달받은 props의 데이터의 type(자료형)을 검사해준다
    자식컴포넌트에서 명시해 놓은 데이터타입과 부모로부터 넘겨받은 데이터 타입이 일치하지 않으면 콘솔에 경고를 띄워줌
*/

class PropsDatatype extends Component {
    render(){
        let {String, Number, Boolean, Object, Array, Function, StringDefault} = this.props;
