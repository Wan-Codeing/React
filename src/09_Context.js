/* 
    ...
    불필요하게 코드를 작성해줘야하는데, 이때 Context를 사용하면 데이터 공급차와, 소비차를 정의하여 데이터가
    필요한 컴포넌트만 사용할수 있게 구현할수 있다

    Context를 사용하면 하위컴포넌트가 여러개인 구조에서 유용하게 사용할수 있음
    먼 후손 컴포넌트도 소비자를 import해서 필요한 데이터를 쓸수 있다

    props VS Context
    props는 부모컴포넌트에서 직계자식컴포넌트에게만 전달되는 데이터라면,
    Context는 모든컴포넌트에 전반적으로 영향을 끼칠수 있다
*/
