/*
    ...
    Route는 호출되는 url에 따라 이동하고자하는 Componet를 정의함
    ...
    리액트에서는 url이동이 서버에서 forwarding해주거나, redirect해주는것이 아니라
    location객체, history객체를 이용해서 클라이언트측에서 페이지 이동이 이루어지는것처럼 보이게함(Client Side Routing CSR)
*/
...
function RouterComponent() {
    return (
        <div>
            <h1>헤더영역</h1>
            {/* Route를 사용하기위해서는 BrowserRouter라는 태그로 감싸줘야함 (보통 최상위 컴포넌트에 추가함) */}
            <BrowserRouter>
                <Routes>
                    {/*
                        path 속성 : 호출되는 url경로
                        element 속성 : url호출시 사용할 컴포넌트를 지정
                    */}
