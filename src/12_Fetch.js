...
/*
    ...
    * 비동기 통신 -> 먼저 시작한 작업의 완료여부와 상관없이 다음 작업을 실행하는 것
    ...
    fetch함수를 이용하여 get/post방식으로 url을호출하여 데이터를 가져온다
*/
function FetchGet() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            */
            ...
            /*
                async ~ await를 붙인 이유?
                fetch함수 , json함수 같은 경우 비동기적으로 작동하기 떄문에 url을 호출하고
                데이터를 가져오기전에 response.json()메서드가 실행되면 에러가 발생할 수 있다
                (json함수는 JSON형태의 문자열 객체에서만 사용가능)

                따라서 데이터를 전부 가져온후에 아래 j0so-++ n함수가 호출되도록 동기적으로 변환시켰다.
            */
